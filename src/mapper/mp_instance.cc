...
static int protoInstSetupEdit(int* pWinId, Object* obj, int* pObjType, int* pObjProtoOff, int* pBufOff, const char* title)
{
    ...
    *pObjProtoOff = 0;
    ...
    windowDrawText(win, title, titleWidth, (kWinWidth - titleWidth) / 2, 18, COLOR_WHITE | FONT_SHADOW);

    // Object type name
    const char* typeName = artGetObjectTypeName(*pObjType);
    int typeNameWidth = fontGetStringWidth(typeName);
    windowDrawText(win, typeName, typeNameWidth, (kWinWidth - typeNameWidth) / 2, 28, COLOR_LIGHT_GREY | FONT_SHADOW);

    // Name button + value
    _win_register_text_button(win, 10, 44, -1, -1, -1, kInstKeyName, "Name", 0);
    const char* objName = objectGetName(obj);
    windowDrawText(win, objName, 130, 90, 48, COLOR_LIGHT_YELLOW  | FONT_SHADOW);
...
static bool regModFlagsDialog(int* flags, int objectType)
{
    ...
    for (int vi = 0; vi < visibleCount; vi++) {
        ...
        windowDrawText(win,
            ...
            kValX,
            yPos + 4,
            flagValues[i] ? (COLOR_LIGHT_YELLOW  | FONT_SHADOW) : (COLOR_GREEN | FONT_SHADOW));
    ...
    while (true) {
        ...
        for (int vi = 0; vi < visibleCount; vi++) {
            ...
            int valColor = flagValues[i] ? (COLOR_LIGHT_YELLOW  | FONT_SHADOW) : (COLOR_GREEN | FONT_SHADOW);
...
static void selectNewScript(Object* obj, int scriptType, int winId, int scriptNameY)
{
    ...
    windowDrawText(winId, scriptName, 130, 90, scriptNameY, COLOR_LIGHT_YELLOW  | FONT_SHADOW);
    windowRefresh(winId);
}
...
static int protoInstAddToInven(int pid, int count)
{
    ...
    if (itemAdd(proto_inst_who_obj, newObj, count) != 0) {
        win_timed_msg("Error adding obj to critter!", COLOR_LIGHT_YELLOW  | FONT_SHADOW);
        return 0;
    }
...
static void protoInstChooseItemsForInvenList(Object* obj)
{
    ...
    int selection = _win_list_select("Pick item to add", names, count, nullptr, 80, 200, COLOR_LIGHT_YELLOW  | FONT_SHADOW);
    if (selection != -1) {
        int quantity = 1;
...
static int protoInstCritterEdit(Object* obj)
{
    ...
    for (;;) {
        ...
        windowDrawText(winId, aiName, 80, 100, kAiY + 4, COLOR_LIGHT_YELLOW  | FONT_SHADOW);
        ...
        windowDrawText(winId, teamStr, 80, 320, kTeamY + 4, COLOR_LIGHT_YELLOW  | FONT_SHADOW);
        ...
        while (true) {
            ...
            if (key == kInstKeyFlags) {
                ...
                windowDrawText(winId, newAiName, 80, 100, kAiY + 4, COLOR_LIGHT_YELLOW  | FONT_SHADOW);
                needRedraw = true;
                ...
                if (win_get_num_i(&team, 0, 32000, false, "Team Num", 100, 100) != -1) {
                    ...
                    snprintf(teamStr, sizeof(teamStr), "%d", obj->data.critter.combat.team);
                    windowDrawText(winId, teamStr, 80, 240, kTeamY + 4, COLOR_LIGHT_YELLOW  | FONT_SHADOW);
                    needRedraw = true;
                }
